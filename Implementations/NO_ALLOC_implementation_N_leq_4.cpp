...
const int MAX_K = 1e5, MAX_M = 1e5;
alignas(64) vec _B[MAX_K], _C[MAX_M];
...
void multiply_N4(double *A, double *B, double *C, int M, int N, int K)
{
    ...
    for(; row + 8 <= M; row += 8)
    {
        // select LR columns of A and LR rows of B to call with the kernel
    ...
    // apply smaller kernels if needed to complete calculation
    if(row + 4 <= M)
    {
        for(int vals = 0; vals < K; vals += LR)
...
double test(int m, int k, int n)
{
    ...
    // use OpenBLAS for n > 4, and our n <= 4 implementation otherwise
    if(n > 4)
        cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasTrans, m, n, k, 1, A, k, B, k, 0, C, n);
    else
        multiply_N4(A, B, D, m, n, k);
